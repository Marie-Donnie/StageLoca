...
- For that, we will probably need
  + projection: unary operation defining a subset of a relation
    - noted \Pi_{a_{1},...,a_{n}}(R)
    - with:
      + a_{1},...,a_{n}: set of attribute names
      + R: the relation from which the subset will be taken
    - result:
      + \Pi_{a_{1}, ...,a_{n}}(R) = {t[a_{1}..,a_n}] : t \in R}
      + with t[a_1,...,a_{n}] = { (a', v) | (a', v) \in t, a' \in a_{1}..,a_{n}
    - example:
  ...
  + selection: unary operation defining a subset of a relation too
    ...
    - in SQL: ~WHERE~
  + (set) union
    - noted R_1 \cup R_2 = {x: x \in R_1 \vee x \in R_2}
  + (set) difference
    - noted R_1 \ R_2 = {x: x \in R_1 \wedge x \notin R_2}
  + intersection
    - noted R_1 \cap R_2 = {x: x \in R_1 \wedge x \in R_2}
  + Cartesian product
    - noted R_1 \times R_2 = {(x,y): x \in R_1 \wedge y \in R_2}
    - in SQL: ~CROSS JOIN~
  + natural join
