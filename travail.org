...
- Configuration changes comes with two phases
  + joint consensus :: combines both configurations
    ...
    - if a leader crash (using the C_{old,new} rules), the new leader may be chosen under C_old or C_{old,new} (C_new cannot make unilateral decisions during this period)
  ...
  + there are no time when C_old or C_new can make unilateral decisions
  ...
  + if the leader is not part of the new configuration, it will become follower has soon as he committed C_new, and will not count for votes
