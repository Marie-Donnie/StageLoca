...
* Tuesday 30 January [2018-01-30 mar.]

- Searching how to use Mariadb/Galera
  + I already noticed [[https://withblue.ink/2016/03/09/galera-cluster-mariadb-coreos-and-docker-part-1.html][this link]], which seems pretty neat
    - I will just make the effort of writing this down so I can remember it: ~--wsrep-new-cluster~ * is used to bootstrap the Galera Cluster. DO NOT USE IT AGAIN ONCE THE CLUSTER HAS BEEN INITIALIZED*
  + Reading the article while deploying
    - [[https://hub.docker.com/_/mariadb/][Pulling mariadb]] ~docker pull mariadb~ on each node (39, 47, 50)
    - 39, 47, 50: ~mkdir -p /opt/local/etc/mysql.conf.d~
    - 39, 47, 50: edit ~/opt/local/etc/mysql.conf.d/mysql_server.cnf~
      #+BEGIN_EXAMPLE
#
...
wsrep_cluster_address="gcomm://172.16.96.39,172.16.96.47,172.16.96.50"
...
# innodb_flush_log_at_trx_commit=0
    ...
    - Make dirs:
      + 39: ~mkdir -p /mnt/data~
      + 47, 50: ~mkdir -p /mnt/data/mysql~
    - Docker run:
      + 39:
	#+BEGIN_SRC
docker run   --name mariadb-0   -d   -v /opt/local/etc/mysql.conf.d:/etc/mysql/conf.d   -v /mnt/data:/var/lib/mysql   -e MYSQL_INITDB_SKIP_TZINFO=yes   -e MYSQL_ROOT_PASSWORD=my-secret-pw   -p 3306:3306   -p 4567:4567/udp   -p 4567-4568:4567-4568   -p 4444:4444   mariadb   --wsrep-new-cluster   --wsrep_node_address=$(ip -4 addr ls eth0 | awk '/inet / {print $2}' | cut -d"/" -f1)
	#+END_SRC
      + 47, 50:
	#+BEGIN_SRC
docker run   --name mariadb-1   -d   -v /opt/local/etc/mysql.conf.d:/etc/mysql/conf.d   -v /mnt/data:/var/lib/mysql   -p 3306:3306   -p 4567:4567/udp   -p 4567-4568:4567-4568   -p 4444:4444   mariadb   --wsrep_node_address=$(ip -4 addr ls eth0 | awk '/inet / {print $2}' | cut -d"/" -f1)
	#+END_SRC
    - Checking db (39, 47, 50):
      + ~export MYSQL_IP=$(ip -4 addr ls eth0 | awk '/inet / {print $2}' | cut -d"/" -f1)~
      + ~docker run --rm -it mariadb mysql -h $MYSQL_IP -u root -p~ with the password defined when running first docker (my-secret-pw)
    - One set of data:
      + 39:
	#+BEGIN_EXAMPLE
...
MariaDB [(none)]> CREATE TABLE bank.accounts (id INT PRIMARY KEY, balance DECIMAL);
Query OK, 0 rows affected (0.11 sec)

MariaDB [(none)]> INSERT INTO bank.accounts VALUES (1, 1000.50);
Query OK, 1 row affected, 1 warning (0.04 sec)

MariaDB [(none)]> SELECT * FROM bank.accounts;
+----+---------+
| id | balance |
+----+---------+
|  1 | 1001    |
...
1 row in set (0.00 sec)
	#+END_EXAMPLE
  + It already works, now I have to make it work using ansible

- I redeploy entirely since I don't want to destroy everything manually

#+BEGIN_EXAMPLE
 'groups': {
u'control': [u'paravance-41.rennes.grid5000.fr'],
'all': [u'paravance-41.rennes.grid5000.fr', u'paravance-44.rennes.grid5000.fr', u'paravance-42.rennes.grid5000.fr', u'paravance-43.rennes.grid5000.fr'],
u'ombt-client': [u'paravance-44.rennes.grid5000.fr'],
u'bus': [u'paravance-42.rennes.grid5000.fr'],
u'mariadb-master': [u'paravance-41.rennes.grid5000.fr'],
'ungrouped': [],
u'chrony': [u'paravance-42.rennes.grid5000.fr', u'paravance-43.rennes.grid5000.fr', u'paravance-44.rennes.grid5000.fr'],
u'ombt-control': [u'paravance-41.rennes.grid5000.fr'],
u'registry': [u'paravance-41.rennes.grid5000.fr'],
u'ombt-server': [u'paravance-43.rennes.grid5000.fr'],
u'control-bus': [u'paravance-41.rennes.grid5000.fr'],
u'mariadb': [u'paravance-42.rennes.grid5000.fr', u'paravance-43.rennes.grid5000.fr', u'paravance-44.rennes.grid5000.fr'],
u'chrony-server': [u'paravance-41.rennes.grid5000.fr']}
#+END_EXAMPLE

- I use groups[all] list, which I put into a list so I can iterate on it in the template
- It's worth noting that you can't use a port range in the ansible ~docker_container~ module.
  - It was easy here because there are only two ports, but I could have use ~with_sequence: start=<startport> end=<endport>~


* Wednesday 31 January [2018-01-31 mer.]

- I resume my tries to make MariaDB work on a cluster with enos-lib

- Trying to put the conf file directly in volume because I'm not sure if it is used (can't find it in /etc/mysql/conf.d)
  - No more luck, it still act like there are no cluster on the nodes that didn't bootstrap it
    #+BEGIN_EXAMPLE
    2018-01-31  9:45:23 140658217932672 [Warning] WSREP: no nodes coming from prim view, prim not possible
2018-01-31  9:45:23 140658217932672 [Note] WSREP: view(view_id(NON_PRIM,73464950,1) memb {
	73464950,0
} joined {
} left {
} partitioned {
})
2018-01-31  9:45:23 140658217932672 [Warning] WSREP: last inactive check more than PT1.5S ago (PT3.50207S), skipping check
2018-01-31  9:45:53 140658217932672 [Note] WSREP: view((empty))
2018-01-31  9:45:53 140658217932672 [ERROR] WSREP: failed to open gcomm backend connection: 110: failed to reach primary view: 110 (Connection timed out)
	 at gcomm/src/pc.cpp:connect():158
2018-01-31  9:45:53 140658217932672 [ERROR] WSREP: gcs/src/gcs_core.cpp:gcs_core_open():208: Failed to open backend connection: -110 (Connection timed out)
2018-01-31  9:45:53 140658217932672 [ERROR] WSREP: gcs/src/gcs.cpp:gcs_open():1458: Failed to open channel 'my_wsrep_cluster' at 'gcomm://172.16.96.40,172.16.96.72,172.16.96.41,172.16.96.70': -110 (Connection timed out)
2018-01-31  9:45:53 140658217932672 [ERROR] WSREP: gcs connect failed: Connection timed out
2018-01-31  9:45:53 140658217932672 [ERROR] WSREP: wsrep::connect(gcomm://172.16.96.40,172.16.96.72,172.16.96.41,172.16.96.70) failed: 7
2018-01-31  9:45:53 140658217932672 [ERROR] Aborting
    #+END_EXAMPLE

- I am beginning to think I forgot something I had to tweak so it could work, so I redeploy without any mariadb and retry the tutorial
  + Everything works fine in less than 5 minutes... :-/

- Trying again, with quotes in the yml file, this time
  + Still no good, but I didn't expect much
  + BUT by comparing the informations about launching a mariadb docker with or without ansible (with ~docker inspect <container-name>, I noticed some differences:
    - For each "PortBindings", in ansible the hostIP is "0.0.0.0", and "" for the command version
    - No "4567/tcp": {}, in the ansible version for the ExposedPorts
    - Strangely, there are no =/etc/mysql/conf.d= in the volumes of the command version
    - In the ansibles versions, the binds (volume) are in "rw" mode, and in the command version it's ""
  + So I have different things to test, I've put them in order of likeliness of causing problems
    #+BEGIN_QUOTE
    Exposed ports are ports that a container or service is using either to provide a service, or listen on. By default, exposed ports in Docker Cloud are only privately accessible. This means only other services that are linked to the service which is exposing the ports can communicate over the exposed port.
...
- Tried with exposed ports, no luck
- Tried with published ports with 4567 before the udp, no luck either
  #+BEGIN_EXAMPLE
              "ExposedPorts": {
                "3306/tcp": {},
                "4444/tcp": {},
  ...
  #+END_EXAMPLE
- I'll try with the port exposed also
  + got ="4567:4567/tcp": {},= I think I'm getting somewhere, trying to put only 4567 on exposed ports
  + Ok the container didn't crashed!
  + Checking the logs
    #+BEGIN_EXAMPLE
    2018-01-31 14:30:03 139866562279296 [Note] mysqld: ready for connections.
Version: '10.2.12-MariaDB-10.2.12+maria~jessie'  socket: '/var/run/mysqld/mysqld.sock'  port: 3306  mariadb.org binary distribution
2018-01-31 14:30:05 139866121680640 [Note] WSREP: 1.0 (c779db4b07c1): State transfer from 2.0 (ade329ad4016) complete.
2018-01-31 14:30:05 139866121680640 [Note] WSREP: Member 1.0 (c779db4b07c1) synced with group.
    #+END_EXAMPLE
  + Getting really excited, checking mysql stuff
  + OMGBBQ it works!
    - on the bootstrapper:
      #+BEGIN_EXAMPLE
MariaDB [(none)]> CREATE DATABASE bank;
Query OK, 1 row affected (0.02 sec)
      #+END_EXAMPLE
    - on a random other node:
      #+BEGIN_EXAMPLE
MariaDB [(none)]> CREATE TABLE bank.accounts (id INT PRIMARY KEY, balance DECIMAL);
Query OK, 0 rows affected (0.12 sec)

MariaDB [(none)]> INSERT INTO bank.accounts VALUES (1, 1000);
...
- [[https://github.com/Marie-Donnie/ombt-orchestrator/commit/cb1edc06fbb6ab6d585dc79720be0b2a9ca64f02][Adding changes to files to the repo]]

- Trying to deploy from scratch, just to be sure
  + And everything is fine of all nodes

- Now I have some cleanup to make so I won't use ombt anymore
  + Will have to make a new repo
  + Doesn't deploy at the moment
    - I will probably have to see what usually happens and check differences


* Thursday 1 February [2018-02-01 jeu.]

- Ronan asked me to make my version built upon enoslib rather than removing things from ombt-orchestrator, so I have to work on that

- Beginning to search how ombt-orchestrator uses enoslib.
  + It seems that it imports libraries.

- Creating [[http://sci.esa.int/juice/][juice]]
  + Updated gitignore
  + Made requirements
  + Copied main.py
  + Began cli.py, copied most of the other
  + Same with tasks.py
  + Copied the roles I made and the ones changed yesterday
  + Copied utils folder because it seemed mandatory
  + Pushed to the repo
  + Added readme
- Launched juice on Nantes because there was no room left at Rennes or Nancy
  + Used cockroachdb in the config file
  + Seems to work
    - Finished without errors
    - ~docker ps~ shows the coachroach containers are up
    - ~docker exec -it cockroachdb-master ./cockroach sql --insecure~ ok
    - [[https://www.cockroachlabs.com/docs/stable/start-a-local-cluster-in-docker.html#step-4-test-the-cluster][cockroachdb test for the cluster]] ok!

- Now I'm trying to enable the choice between cockroach or maria db at launch
  + Been thinking about making some replacement in the dict but it was really crappy
  + I asked Ronan about a proper way he told me I could change it in ~site.yml~:
    - the inventory specifies the hosts where roles are deployed:
      #+BEGIN_EXAMPLE
[db-master]
econome-12.nantes.grid5000.fr ansible_host=econome-12.nantes.grid5000.fr ansible_ssh_user=root ansible_ssh_common_args='-o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null' internal_network=eth0 control_network=eth0 enos_devices="['eth0']"
      #+END_EXAMPLE
    - so I can change in the ~site.yml~ file the desired hosts type for each role AND THEN
    - add a ~when~ with an environment variable


* Friday 2 February [2018-02-02 ven.]

- Going to try what I pushed yesterday right now

** Option patch

- It worked with ~./cli.py deploy~, using MariaDB as default database
  #+BEGIN_EXAMPLE
PLAY [CockroachDB master deployment] *******************************************************************

TASK [cockroachdb-master : include] ********************************************************************
skipping: [parapluie-25.rennes.grid5000.fr]

PLAY [CockroachDB deployment] **************************************************************************

TASK [cockroachdb : include] ***************************************************************************
skipping: [parapluie-8.rennes.grid5000.fr]
skipping: [parapluie-31.rennes.grid5000.fr]
skipping: [parapluie-38.rennes.grid5000.fr]

PLAY [MariaDB master deployment] ***********************************************************************

TASK [mariadb-master : include] ************************************************************************
included: /home/madelavergne/juice/ansible/roles/mariadb-master/tasks/deploy.yml for parapluie-25.rennes.grid5000.fr

TASK [mariadb-master : Create the directories] *********************************************************
changed: [parapluie-25.rennes.grid5000.fr] => (item=/opt/local/etc/mysql.conf.d/)
changed: [parapluie-25.rennes.grid5000.fr] => (item=/mnt/data)

TASK [mariadb-master : Generate the configuration file] ************************************************
changed: [parapluie-25.rennes.grid5000.fr] => (item=[u'parapluie-25.rennes.grid5000.fr', u'parapluie-8.rennes.grid5000.fr', u'parapluie-31.rennes.grid5000.fr', u'parapluie-38.rennes.grid5000.fr'])

TASK [mariadb-master : Start the MariaDB-master container] *********************************************
changed: [parapluie-25.rennes.grid5000.fr]

PLAY [MariaDB deployment] ******************************************************************************

TASK [mariadb : include] *******************************************************************************
included: /home/madelavergne/juice/ansible/roles/mariadb/tasks/deploy.yml for parapluie-8.rennes.grid5000.fr, parapluie-31.rennes.grid5000.fr, parapluie-38.rennes.grid5000.fr

TASK [mariadb : Create the directories] ****************************************************************
changed: [parapluie-8.rennes.grid5000.fr] => (item=/opt/local/etc/mysql.conf.d/)
changed: [parapluie-31.rennes.grid5000.fr] => (item=/opt/local/etc/mysql.conf.d/)
changed: [parapluie-38.rennes.grid5000.fr] => (item=/opt/local/etc/mysql.conf.d/)
changed: [parapluie-8.rennes.grid5000.fr] => (item=/mnt/data/mysql)
changed: [parapluie-31.rennes.grid5000.fr] => (item=/mnt/data/mysql)
changed: [parapluie-38.rennes.grid5000.fr] => (item=/mnt/data/mysql)

TASK [mariadb : Generate the configuration file] *******************************************************
changed: [parapluie-8.rennes.grid5000.fr] => (item=[u'parapluie-25.rennes.grid5000.fr', u'parapluie-8.rennes.grid5000.fr', u'parapluie-31.rennes.grid5000.fr', u'parapluie-38.rennes.grid5000.fr'])
changed: [parapluie-31.rennes.grid5000.fr] => (item=[u'parapluie-25.rennes.grid5000.fr', u'parapluie-8.rennes.grid5000.fr', u'parapluie-31.rennes.grid5000.fr', u'parapluie-38.rennes.grid5000.fr'])
changed: [parapluie-38.rennes.grid5000.fr] => (item=[u'parapluie-25.rennes.grid5000.fr', u'parapluie-8.rennes.grid5000.fr', u'parapluie-31.rennes.grid5000.fr', u'parapluie-38.rennes.grid5000.fr'])

TASK [mariadb : Start the MariaDB container] ***********************************************************
changed: [parapluie-8.rennes.grid5000.fr]
changed: [parapluie-31.rennes.grid5000.fr]
changed: [parapluie-38.rennes.grid5000.fr]
  #+END_EXAMPLE
- Then I used ~./cli.py deploy --db=cockroachdb~
  #+BEGIN_EXAMPLE
  PLAY [CockroachDB master deployment] *******************************************************************

TASK [cockroachdb-master : include] ********************************************************************
included: /home/madelavergne/juice/ansible/roles/cockroachdb-master/tasks/deploy.yml for parapluie-25.rennes.grid5000.fr

TASK [cockroachdb-master : Start cockroachdb master] ***************************************************
changed: [parapluie-25.rennes.grid5000.fr]

PLAY [CockroachDB deployment] **************************************************************************

TASK [cockroachdb : include] ***************************************************************************
included: /home/madelavergne/juice/ansible/roles/cockroachdb/tasks/deploy.yml for parapluie-8.rennes.grid5000.fr, parapluie-31.rennes.grid5000.fr, parapluie-38.rennes.grid5000.fr

TASK [cockroachdb : Start cockroachdb] *****************************************************************
fatal: [parapluie-8.rennes.grid5000.fr]: FAILED! => {"failed": true, "msg": "The task includes an option with an undefined variable. The error was: 'dict object' has no attribute 'cockroachdb-master'\n\nThe error appears to have been in '/home/madelavergne/juice/ansible/roles/cockroachdb/tasks/deploy.yml': line 2, column 3, but may\nbe elsewhere in the file depending on the exact syntax problem.\n\nThe offending line appears to be:\n\n---\n- name: Start cockroachdb\n  ^ here\n"}
fatal: [parapluie-31.rennes.grid5000.fr]: FAILED! => {"failed": true, "msg": "The task includes an option with an undefined variable. The error was: 'dict object' has no attribute 'cockroachdb-master'\n\nThe error appears to have been in '/home/madelavergne/juice/ansible/roles/cockroachdb/tasks/deploy.yml': line 2, column 3, but may\nbe elsewhere in the file depending on the exact syntax problem.\n\nThe offending line appears to be:\n\n---\n- name: Start cockroachdb\n  ^ here\n"}
fatal: [parapluie-38.rennes.grid5000.fr]: FAILED! => {"failed": true, "msg": "The task includes an option with an undefined variable. The error was: 'dict object' has no attribute 'cockroachdb-master'\n\nThe error appears to have been in '/home/madelavergne/juice/ansible/roles/cockroachdb/tasks/deploy.yml': line 2, column 3, but may\nbe elsewhere in the file depending on the exact syntax problem.\n\nThe offending line appears to be:\n\n---\n- name: Start cockroachdb\n  ^ here\n"}
  #+END_EXAMPLE
  + Cockroach-master worked fined but there was an error while deploying cockroach "slaves"
  + Of course there was, the deploy file for cockroach was trying to join 'cockroachdb-master'
    #+BEGIN_SRC
    "{{ hostvars[groups['cockroachdb-master'][0]]['ansible_' + control_network]['ipv4']['address'] }}"
    #+END_SRC
  + Changed it to db-master
    #+BEGIN_SRC
    PLAY [CockroachDB master deployment] *******************************************************************
...
- Checking the databases:
  + cockroachdb:
    ...
    - [[https://www.cockroachlabs.com/docs/stable/start-a-local-cluster-in-docker.html#step-4-test-the-cluster][cockroachdb test for the cluster]] ok
  + mariadb
    - tried to check whether it was ok to use ~./cli.py deploy --db=mariadb~ ok
    - ~docker ps~ ok
    - ~docker run --rm -it mariadb mysql -h 172.16.99.9 -u root -p~ ok
    - [[https://www.cockroachlabs.com/docs/stable/start-a-local-cluster-in-docker.html#step-4-test-the-cluster][cockroachdb test for the cluster]] ok

** Towards sysbench up and running

- Now I can search how to make the required changes
  + On the [[https://hub.docker.com/r/severalnines/sysbench/][sysbench docker page]], it says as on the github page for sysbench, to create a database that will be used by sysbench

*** MariaDB
  + On the [[https://hub.docker.com/_/mariadb/][mariadb docker page]], it reads:
  ...
  + Maybe if I use it when creating the mariadb docker to specify sbtest it will be enough
  + The db has been created
    - [[https://www.cockroachlabs.com/docs/stable/start-a-local-cluster-in-docker.html#step-4-test-the-cluster][cockroachdb test for the cluster]] ok
      #+BEGIN_EXAMPLE
root@parapluie-20:~# docker run --rm -it mariadb mysql -h 172.16.99.31 -u sbtest -p
Enter password:
...
MariaDB [sbtest]> CREATE TABLE sbtest.accounts (id INT PRIMARY KEY, balance DECIMAL);
Query OK, 0 rows affected (0.17 sec)

MariaDB [sbtest]> INSERT INTO sbtest.accounts VALUES (1, 1000.50);
Query OK, 1 row affected, 1 warning (0.05 sec)

MariaDB [sbtest]> SELECT * FROM sbtest.accounts;
+----+---------+
| id | balance |
+----+---------+
|  1 | 1001    |
...
*** Connecting sysbench to MariaDB

- Now I have to connect sysbench to MariaDB
  + Concerning the tests I can run:
    - =oltp_bulk_insert=
    - =oltp_delete=
    - =oltp_insert=
    - =oltp_point_select=
...
- Wanted to try and make something not too dirty for the command config ([[https://stackoverflow.com/questions/36859662/ansible-conditional-module-arguments][something like this]]), but didn't work, so I'm doing it with two plays and whens

- Shouldn't have done it with MariaDB first because when relaunching the deployment, it doesn't work since there is already a cluster
  + should try and see if I can destroy a cluster
    - seems that there are no proper way [[https://dba.stackexchange.com/questions/151941/how-to-restart-mariadb-galera-cluster][but some ways]]

- It finally works:
#+BEGIN_EXAMPLE
root@parapluie-1:~# docker run --rm -it severalnines/sysbench sysbench --db-driver=mysql --table-size=100000 --mysql-host=172.16.99.1 --mysql-port=3306 --mysql-user=sbtest --mysql-password=sbtest oltp_read_write run
...
SQL statistics:
    queries performed:
        ...
        other:                           789
        total:                           3940
    transactions:                        197    (19.61 per sec.)
    queries:                             3940   (392.23 per sec.)
...
Threads fairness:
    events (avg/stddev):           197.0000/0.00
    execution time (avg/stddev):   10.0408/0.00
#+END_EXAMPLE

+ I'll just have to had the =run= command after that and get the reports
+ But I will also have to try to find a better way to separate cockroachdb and mariadb version or I will have to make two plays of everything

+ Finishing my work on this cliffhanger


** Meeting with Ronan about summit proposal
...
- on devstack, default token are fernet, on kolla-ansible it's uuid
- we might use the experiments to see the effect of split-brain (and reconnection to the cluster)

*** Links

- [[https://www.openstack.org/videos/sydney-2017/demystifying-identity-federation][Talk from Colleen Murphy about Identity Federation]]
- [[https://developer.rackspace.com/blog/keystone-to-keystone-federation-with-openstack-ansible/][Blog article about K2K federation]]
- [[http://lists.openstack.org/pipermail/openstack-dev/2017-May/117659.html][Jay Pipes answer about CockroachDB for Keystone]] (what we will need to test: a large number of reads, few writes)
- [[http://lists.openstack.org/pipermail/openstack-dev/2017-May/117659.html][Blog article about Fernet token format for KS]]
