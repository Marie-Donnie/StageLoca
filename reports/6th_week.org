...
* Wednesday 21 February [2018-02-21 mer.]

- Today I had to quickly explain to a student from middle school -they have a 3 day 'internship' to see how people work in a domain- the university, the bachelor and master's degrees

- I have now an error when trying to launch ~./juice.py deploy~
  #+BEGIN_EXAMPLE
DEBUG:urllib3.connectionpool:https://api-ext.grid5000.fr:443 "GET /3.0/sites HTTP/1.1" 503 397
Traceback (most recent call last):
  File "./juice.py", line 261, in <module>
    doc_lookup(args['<command>'], argv)
  File "/home/madelavergne/juice/utils/doc.py", line 26, in doc_lookup
    return fn(**docopt(fn.__doc__, argv=argv))
  File "/home/madelavergne/juice/utils/doc.py", line 18, in decorated
    fn(*args, **kwargs)
  File "./juice.py", line 66, in deploy
    g5k(config=config)
  File "/home/madelavergne/juice/utils/doc.py", line 18, in decorated
    fn(*args, **kwargs)
  File "/home/madelavergne/juice/venv/local/lib/python2.7/site-packages/enoslib/task.py", line 50, in decorated
    fn(*args, **kwargs)
  File "./juice.py", line 80, in g5k
  ...
  File "/home/madelavergne/juice/venv/local/lib/python2.7/site-packages/enoslib/infra/enos_g5k/api.py", line 81, in launch
    self.reserve(**kwargs)
  File "/home/madelavergne/juice/venv/local/lib/python2.7/site-packages/enoslib/infra/enos_g5k/api.py", line 93, in reserve
    reservation_date)
  ...
  File "/home/madelavergne/juice/venv/local/lib/python2.7/site-packages/execo_g5k/api_utils.py", line 270, in get_api_data
    _data = _get_api()
  File "/home/madelavergne/juice/venv/local/lib/python2.7/site-packages/execo_g5k/api_utils.py", line 360, in _get_api
    for site in sorted(_get_g5k_sites_uncached()):
  File "/home/madelavergne/juice/venv/local/lib/python2.7/site-packages/execo_g5k/api_utils.py", line 257, in _get_g5k_sites_uncached
...
execo_g5k.api_utils.APIException: <APIException uri='https://api-ext.grid5000.fr/3.0/sites' method=GET status=503/"Service Unavailable" content="<!DOCTYPE HTML PUBLIC "-//IETF//DTD HTML 2.0//EN">
<html><head>
...
- Seems it solved itself...

- Deployment of Openstack worked on "slave" nodes, but not on master
#+BEGIN_EXAMPLE
++ functions-common:oscwrap:2537            :   start=1519210576793
+++ functions-common:oscwrap:2538            :   command openstack role assignment list --role member --user 14c75c79de61446fb3a12ef57149b540 --project
+++ functions-common:oscwrap:2538            :   openstack role assignment list --role member --user 14c75c79de61446fb3a12ef57149b540 --project
...
++ functions-common:oscwrap:2538            :   out=
++ functions-common:oscwrap:2539            :   rc=2
+++ functions-common:oscwrap:2540            :   date +%s%3N
++ functions-common:oscwrap:2540            :   end=1519210577861
++ functions-common:oscwrap:2541            :   echo 1068
++ functions-common:oscwrap:2543            :   echo ''
++ functions-common:oscwrap:2544            :   return 2
+ functions-common:get_or_add_user_project_role:871 :   user_role_id=
+ functions-common:get_or_add_user_project_role:872 :   [[ -z '' ]]
+ functions-common:get_or_add_user_project_role:874 :   openstack role add member --user 14c75c79de61446fb3a12ef57149b540 --project
...
+ functions-common:get_or_add_user_project_role:1 :   exit_trap
+ stack.sh:exit_trap:521                   :   local r=2
++ stack.sh:exit_trap:522                   :   jobs -p
+ stack.sh:exit_trap:522                   :   jobs=
+ stack.sh:exit_trap:525                   :   [[ -n '' ]]
+ stack.sh:exit_trap:531                   :   '[' -f /tmp/tmp.gun4nbKXB0 ']'
+ stack.sh:exit_trap:532                   :   rm /tmp/tmp.gun4nbKXB0
+ stack.sh:exit_trap:536                   :   kill_spinner
+ stack.sh:kill_spinner:417                :   '[' '!' -z '' ']'
+ stack.sh:exit_trap:538                   :   [[ 2 -ne 0 ]]
+ stack.sh:exit_trap:539                   :   echo 'Error on exit'
Error on exit
+ stack.sh:exit_trap:540                   :   generate-subunit 1519209972 606 fail
+ stack.sh:exit_trap:541                   :   [[ -z /opt/stack/logs ]]
+ stack.sh:exit_trap:544                   :   /opt/stack/devstack/tools/worlddump.py -d /opt/stack/logs
/bin/sh: 1: arp: not found
...
- When I relaunched the experiment, there was a transaction problem again... And again. I will need to find a better workaround or fix.

- It was way more difficult than I thought to make scenarios work from a directory
  + I also discovered how to remove last x containers: ~docker rm $(docker ps -aqn x)~
  + It seems to work but I will have to get the reports also


* Thursday 22 February [2018-02-22 jeu.]
...
- Have to get the reports
  + during ~juice openstack~, I got
    #+BEGIN_EXAMPLE
    INFO migrate.versioning.api [-] 90 -> 91...
CRITICAL keystone [-] Unhandled error: DbMigrationError: (psycopg2.extensions.TransactionRollbackError) restart transaction: HandledRetryableTxnError: TransactionRetryError: retry txn (RETRY_SERIALIZABLE): "sql txn" id=2965edd6 key=/Table/SystemConfigSpan/Start rw=true pri=0.03802320 iso=SERIALIZABLE stat=PENDING epo=0 ts=1519305600.672679962,1 orig=1519305600.671018918,0 max=1519305600.671018918,0 wto=false rop=false seq=13
ERROR keystone Traceback (most recent call last):
ERROR keystone   File "/usr/local/bin/keystone-manage", line 10, in <module>
ERROR keystone     sys.exit(main())
ERROR keystone   File "/opt/stack/keystone/keystone/cmd/manage.py", line 45, in main
...
+ stack.sh:exit_trap:522                   :   jobs=
+ stack.sh:exit_trap:525                   :   [[ -n '' ]]
+ stack.sh:exit_trap:531                   :   '[' -f /tmp/tmp.dLdvutYINB ']'
+ stack.sh:exit_trap:532                   :   rm /tmp/tmp.dLdvutYINB
+ stack.sh:exit_trap:536                   :   kill_spinner
+ stack.sh:kill_spinner:417                :   '[' '!' -z '' ']'
+ stack.sh:exit_trap:538                   :   [[ 1 -ne 0 ]]
+ stack.sh:exit_trap:539                   :   echo 'Error on exit'
Error on exit
+ stack.sh:exit_trap:540                   :   generate-subunit 1519305094 507 fail
+ stack.sh:exit_trap:541                   :   [[ -z /opt/stack/logs ]]
+ stack.sh:exit_trap:544                   :   /opt/stack/devstack/tools/worlddump.py -d /opt/stack/logs
/bin/sh: 1: arp: not found
+ stack.sh:exit_trap:550                   :   exit 1
    #+END_EXAMPLE
  + Ran smoothly using ~sleep $(( ( RANDOM % 20 )  + 50 ))~. Maybe I got lucky, but I was afraid at first random because both nodes decided to sleep 59

- Also adding the ~info~ command to juice
  + got difficulties since it uses a parameter but [[https://stackoverflow.com/questions/10307696/how-to-put-a-variable-into-python-docstring][this]] helped me
  + now ~./juice.py info~ works, this might be handy \o/

- Maybe I will be able to download libmysqlclient-dev using [[https://askubuntu.com/questions/952113/how-to-bypass-dpkg-prompt][noninteractive]]

- Worked a bit on the README file, which helped me to see where there are/were still problems to use juice


* Friday 23 February [2018-02-23 ven.]

** Juicy work

- Deploying. Nervous about openstack deployment
  + ~DEBIAN_FRONTEND=noninteractive~ worked
  + still had a transaction error, and a second time
  + tried to connect using ssh from one node to another to see if I can just grep my way out of this, but the nodes can't communicate this way
  + Finally worked after a dozen of tries and changing sleep time to ~sleep $(( ( RANDOM % 30 )  + 60 ))~

- Added a tasks ran to know what has been used and backup/destroy only if necessary
- Added tags to skip ~ansible-playbook example.yml --skip-tags "notification"~ or run only those tags ~ansible-playbook example.yml --tags "configuration,packages"~

- Trying to run devstack on MariaDB
  + Multiple problems:
    - Had to create a lib-galera.j2
    - Adapt deployment of openstack
    - Adapt local.conf.j2 to enable required service depending of the parameter given
    - Keystone wanted to use a mariadb driver
      + needed to echo 'mysql' in ~get_database_type_galera~ (got stuck A LOT of time on this...)
    - Nodes also needed to sleep like for cockroach, so I had to change the commands
- But now it works!

** Other

- Found a [[https://www.cockroachlabs.com/blog/database-evolution/][CockroachDB demo]]
